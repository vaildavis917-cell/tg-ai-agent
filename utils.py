...
def get_moscow_hour() -> int:
    moscow_now = _utcnow() + timedelta(hours=TIMEZONE_OFFSET)
    return moscow_now.hour

def get_moscow_now() -> datetime:
    return _utcnow() + timedelta(hours=TIMEZONE_OFFSET)
...
def is_appropriate_time_for_user(country: str = None) -> bool:
    ...
    if country:
        ...
        if user_offset is not None:
            user_hour = (_utcnow() + timedelta(hours=user_offset)).hour
...
def get_healthcheck_data() -> dict:
    ...
    if _healthcheck_data["started_at"]:
        ...
        # Если started naive — добавляем UTC
        if started.tzinfo is None:
            started = started.replace(tzinfo=timezone.utc)
        _healthcheck_data["uptime_seconds"] = int((_utcnow() - started).total_seconds())
