...
from storage import (
    ...
    periodic_conversations_save_loop,
)
from prompts import load_ab_tests
from analytics import load_analytics, save_analytics, periodic_analytics_save_loop
...
async def graceful_shutdown():
    """Сохраняет все данные и отключает клиент при остановке."""
    logger.info("Graceful shutdown: сохраняю данные...")
    try:
        save_conversations()
        save_leads_map()
        save_followup()
        save_lead_statuses()
        save_analytics()
        logger.info("Graceful shutdown: все данные сохранены")
    except Exception as e:
        logger.error(f"Ошибка при graceful shutdown: {e}")
    try:
        await client.disconnect()
        logger.info("Graceful shutdown: клиент отключён")
    except Exception:
        pass
    _shutdown_event.set()
...
async def main():
    ...
    # Регистрация signal handlers для graceful shutdown (#18)
    ...
    asyncio.create_task(periodic_analytics_save_loop())
    asyncio.create_task(periodic_conversations_save_loop())
    ...
    # Работаем до отключения или shutdown
