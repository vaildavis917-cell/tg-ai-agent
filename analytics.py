...
# Dirty flag для debounce-сохранения
...
def save_analytics():
    """Сохраняет аналитику на диск (thread-safe через _write_json)."""
    try:
        _write_json(ANALYTICS_FILE, analytics_data)
    except Exception as e:
        logger.error(f"Ошибка сохранения аналитики: {e}")

def _mark_dirty():
    """Помечает аналитику как изменённую (сохранение в periodic loop)."""
    global _analytics_dirty
    _analytics_dirty = True

async def periodic_analytics_save_loop():
    """Фоновая задача: сохраняет аналитику каждые 30 секунд если были изменения."""
    global _analytics_dirty
    while True:
        try:
            ...
            logger.error(f"Ошибка periodic_analytics_save_loop: {e}")


# ============ ТРЕКИНГ СОБЫТИЙ (debounce — не пишем на диск каждый раз) ============
...
def get_analytics_report() -> str:
    ...
    # Вчера
    yesterday = (datetime.now(timezone.utc) + timedelta(hours=TIMEZONE_OFFSET) - timedelta(days=1)).strftime("%Y-%m-%d")
