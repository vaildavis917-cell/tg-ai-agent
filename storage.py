...
def load_daily_counts():
    """Загружает дневные счётчики с диска (переживают перезапуск)"""
    ...
    try:
        if os.path.exists(DAILY_COUNTS_FILE):
            ...
            if saved_date == str(date.today()):
                daily_message_count = data.get("total", 0)
                daily_user_counts = defaultdict(int, {int(k): v for k, v in data.get("users", {}).items()})
                last_count_date = date.today()
                logger.info(f"Загружены дневные счётчики: {daily_message_count} сообщений")
            else:
                logger.info("Дневные счётчики устарели, начинаем с нуля")
    except Exception as e:
        logger.error(f"Ошибка загрузки дневных счётчиков: {e}")


# ============ СОХРАНЕНИЕ (thread-safe) ============
def _write_json(path: str, data):
    """Atomic-подобная запись: пишем во временный файл, потом rename. Thread-safe."""
    with _write_lock:
        tmp_path = path + ".tmp"
...
def save_daily_counts():
    """Сохраняет дневные счётчики на диск"""
    try:
        data = {
            "date": str(last_count_date),
            "total": daily_message_count,
            "users": {str(k): v for k, v in daily_user_counts.items()},
        }
        _write_json(DAILY_COUNTS_FILE, data)
    except Exception as e:
        logger.error(f"Ошибка сохранения дневных счётчиков: {e}")


# ============ DEBOUNCE: периодическое сохранение conversations ============
def mark_conversations_dirty():
    """Помечает conversations как изменённые (сохранение произойдёт в save_loop)"""
    global _conversations_dirty
    _conversations_dirty = True

async def periodic_conversations_save_loop():
    """Фоновая задача: сохраняет dirty данные каждые 15 секунд"""
    global _conversations_dirty
    while True:
        try:
            await asyncio.sleep(15)
            if _conversations_dirty:
                save_conversations()
                _conversations_dirty = False
        except Exception as e:
            logger.error(f"Ошибка periodic_save_loop: {e}")
