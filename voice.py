...
def add_emotion_tags(text: str) -> str:
    """Добавляет эмоциональные теги для TTS"""
    tagged = text
    if any(w in text.lower() for w in ["привет", "здравствуй", "рад", "добр", "hello", "hi"]):
        tagged = "[friendly] " + tagged
    elif any(w in text.lower() for w in ["peace and love", "stay smart", "удачи", "пока", "до связи"]):
        tagged = "[warm] " + tagged
    elif any(w in text.lower() for w in ["farallon", "команда", "аналитик", "результат", "доходность"]):
        tagged = "[confident] " + tagged
    elif "?" in text:
        tagged = "[curious] " + tagged
    elif ")" in text and len(text) < 100:
        tagged = "[laughs softly] " + tagged
    else:
        tags = ["[calm]", "[casual]", "[thoughtful]"]
        tagged = random.choice(tags) + " " + tagged
    tagged = tagged.replace(". ", "... ", 1) if random.random() < 0.3 else tagged
    logger.info(f"Emotion tags: {tagged[:80]}...")
    return tagged
